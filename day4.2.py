...
test = 'aaaaa-bbb-z-yaabbccddeeffgghhiijjjjkklmnopqrstuvwxyzzz-x-123[abxyz]'
    ...
    for char in i:
        if char.isalpha():
            if char in count:
                ...
                count[char]=1
    frequent = [v[0] for v in sorted(count.items(), key=lambda kv: (-kv[1], kv[0]))]
    result = frequent[:5]
    checksum_test = ''
    for j in result:
        checksum_test+=j
...
for i in valid:
    ...
    for char in i[:-10]:
        if char == '-':
            ...
            decrypted += alphabet[(alphabet.index(char)+cipher)%26]
    x = (decrypted, cipher)
    crypt_list[cipher]=decrypted
print(crypt_list)

print(crypt_list.find("north pole objects"))
